...
public class DoiCoInsuranceController {
	...
	@Trace
	@PostMapping(COINS_POLICY_ENTRY_LISTING_URL+URLConstant.URL_GET_ALL)
	public ResponseEntity<ApiResponse> getPolicyEntryListing(@RequestBody PageDetails pageDetail) throws CustomException, ParseException {
		PagebleDTO<DoiCoinsPolicyHdrDTO> data = service.getPolicyEntryListing(pageDetail);
		return ResponseUtil.getResponse(HttpStatus.OK, MsgConstant.GST_MSG_FETCH_LIST, data);
	}

	@Trace
	@PostMapping(COINS_PREMIUM_REFUND_ENTRY_LISTING_URL+URLConstant.URL_GET_ALL)
	public ResponseEntity<ApiResponse> getPremiumRefundEntryListing(@RequestBody PageDetails pageDetail) throws CustomException, ParseException {
		PagebleDTO<DoiCoinsPremiumRefundDTO> data = service.getPremiumRefundEntryListing(pageDetail);
		return ResponseUtil.getResponse(HttpStatus.OK, MsgConstant.GST_MSG_FETCH_LIST, data);
	}

	@Trace
	@PostMapping(COINS_CLAIM_LISTING_URL+URLConstant.URL_GET_ALL)
	public ResponseEntity<ApiResponse> getClaimListing(@RequestBody PageDetails pageDetail) throws CustomException, ParseException {
		PagebleDTO<DoiCoinsClaimHdrDTO> data = service.getClaimListing(pageDetail);
		return ResponseUtil.getResponse(HttpStatus.OK, MsgConstant.GST_MSG_FETCH_LIST, data);
	}
}
